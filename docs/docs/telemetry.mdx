...
This ping is intentionally separate from telemetry so Wave can count active installs. If you'd like to disable it, set the DORATERM_NOPING environment variable.
...
For the complete, current list of all telemetry events and properties, see the source code: [telemetrydata.go](https://github.com/dfbb/doraterm/blob/main/pkg/telemetry/telemetrydata/telemetrydata.go)
