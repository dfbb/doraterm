...
export type RemoteTarget = { host: string; port: number; baseUrl: string };
...
function parseRemoteHostArg(argv: string[]): RemoteTarget | null {
    ...
    if (value.startsWith("https://") || value.startsWith("http://")) {
        ...
        return { host: url.hostname, port, baseUrl };
    }
    const idx = value.lastIndexOf(":");
    if (idx < 0) {
        return { host: value, port: DEFAULT_PORT, baseUrl: `http://${value}:${DEFAULT_PORT}` };
    }
