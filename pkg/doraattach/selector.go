...
func ListTermBlocks(rpcClient *dshutil.DshRpc) ([]blockEntry, error) {
	allBlocks, err := dshclient.BlocksListCommand(rpcClient, dshrpc.BlocksListRequest{}, nil)
	if err != nil {
		return nil, fmt.Errorf("listing blocks: %w", err)
	}

	wsCache := make(map[string]string)
	if wsList, err := dshclient.WorkspaceListCommand(rpcClient, nil); err == nil {
	...
	for _, blk := range allBlocks {
		view := blk.Meta.GetString(doraobj.MetaKey_View, "")
		if view != "term" {
			continue
		}
		...
		if !ok {
			tab, err := dshclient.GetTabCommand(rpcClient, blk.TabId, nil)
			if err == nil && tab != nil {
				tabName = tab.Name
			}
			if tabName == "" {
		...
		cwd := blk.Meta.GetString(doraobj.MetaKey_CmdCwd, "")
