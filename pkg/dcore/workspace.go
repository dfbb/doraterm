...
func CreateWorkspace(ctx context.Context, name string, icon string, color string, applyDefaults bool, isInitialLaunch bool) (*doraobj.Workspace, error) {
	ws := &doraobj.Workspace{
		OID:    uuid.NewString(),
		TabIds: []string{},
		Name:   "",
		Icon:   "",
...
func UpdateWorkspace(ctx context.Context, workspaceId string, name string, icon string, color string, applyDefaults bool) (*doraobj.Workspace, bool, error) {
...
func DeleteWorkspace(ctx context.Context, workspaceId string, force bool) (bool, string, error) {
	...
	workspace, err := dstore.DBMustGet[*doraobj.Workspace](ctx, workspaceId)
	if err != nil && dstore.ErrNotFound == err {
	...
	windowId, _ := dstore.DBFindWindowForWorkspaceId(ctx, workspaceId)
	err = dstore.DBDelete(ctx, doraobj.OType_Workspace, workspaceId)
	if err != nil {
...
func GetWorkspace(ctx context.Context, wsID string) (*doraobj.Workspace, error) {
	return dstore.DBMustGet[*doraobj.Workspace](ctx, wsID)
}
...
func createTabObj(ctx context.Context, workspaceId string, name string, meta doraobj.MetaMapType) (*doraobj.Tab, error) {
	...
	layoutState := &doraobj.LayoutState{
		OID: layoutStateId,
	}
	ws.TabIds = append(ws.TabIds, tab.OID)
	dstore.DBInsert(ctx, tab)
	dstore.DBInsert(ctx, layoutState)
	dstore.DBUpdate(ctx, ws)
	return tab, nil
}
...
func DeleteTab(ctx context.Context, workspaceId string, tabId string, recursive bool) (string, error) {
	ws, _ := dstore.DBGet[*doraobj.Workspace](ctx, workspaceId)
	if ws == nil {
	...
	tab, _ := dstore.DBGet[*doraobj.Tab](ctx, tabId)
	if tab != nil {
	...
	dstore.DBUpdate(ctx, ws)
	dstore.DBDelete(ctx, doraobj.OType_Tab, tabId)
	if tab != nil {
		dstore.DBDelete(ctx, doraobj.OType_LayoutState, tab.LayoutState)
	}
...
func ListWorkspaces(ctx context.Context) (doraobj.WorkspaceList, error) {
	workspaces, err := dstore.DBGetAllObjsByType[*doraobj.Workspace](ctx, doraobj.OType_Workspace)
	if err != nil {
		return nil, err
	}
	windows, err := dstore.DBGetAllObjsByType[*doraobj.Window](ctx, doraobj.OType_Window)
	if err != nil {
		return nil, err
	}
	...
	for _, workspace := range workspaces {
		...
		wl = append(wl, &doraobj.WorkspaceListEntry{
...
func SetIcon(workspaceId string, icon string) error {
	...
	defer cancel()
	ws, e := dstore.DBGet[*doraobj.Workspace](ctx, workspaceId)
	if e != nil {
		return e
	}
	if ws == nil {
