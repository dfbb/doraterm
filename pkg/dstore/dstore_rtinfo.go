...
var (
	rtInfoStore = make(map[doraobj.ORef]*doraobj.ObjRTInfo)
...
func SetRTInfo(oref doraobj.ORef, info map[string]any) {
	rtInfoMutex.Lock()
	defer rtInfoMutex.Unlock()

	rtInfo, exists := rtInfoStore[oref]
	if !exists {
		rtInfo = &doraobj.ObjRTInfo{}
...
func GetRTInfo(oref doraobj.ORef) *doraobj.ObjRTInfo {
	rtInfoMutex.RLock()
...
func DeleteRTInfo(oref doraobj.ORef) {
	rtInfoMutex.Lock()
	defer rtInfoMutex.Unlock()
