...
import (
	...
	"github.com/dfbb/doraterm/pkg/util/dbutil"
...
func DBGetSingletonByType(ctx context.Context, otype string) (doraobj.DoraObj, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (doraobj.DoraObj, error) {
		...
		rtn, err := doraobj.FromJson(row.Data)
		if err != nil {
			return rtn, err
		}
		doraobj.SetVersion(rtn, row.Version)
		return rtn, nil
	})
}

func DBExistsORef(ctx context.Context, oref doraobj.ORef) (bool, error) {
...
func DBResolveEasyOID(ctx context.Context, oid string) (*doraobj.ORef, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (*doraobj.ORef, error) {
		for _, rtype := range doraobj.AllDoraObjTypes() {
			...
			if fullOID != "" {
				oref := doraobj.MakeORef(otype, fullOID)
				return &oref, nil
			}
		}
		return nil, ErrNotFound
...
func DBSelectMap[T doraobj.DoraObj](ctx context.Context, ids []string) (map[string]T, error) {
	...
	for _, obj := range rtnArr {
		rtnMap[doraobj.GetOID(obj)] = obj.(T)
	}
	return rtnMap, nil
}
...
func DBUpdate(ctx context.Context, val doraobj.DoraObj) error {
	...
	jsonData, err := doraobj.ToJson(val)
	if err != nil {
		return err
	}
...
func DBFindTabForBlockId(ctx context.Context, blockId string) (string, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (string, error) {
		...
		for {
			...
			oref, err := doraobj.ParseORef(parentORef)
			if err != nil {
