...
function _doraterm_si_osc7
    _doraterm_si_blocked; and return
    # Use fish-native URL encoding
    set -l encoded_pwd (string escape --style=url -- "$PWD")
    printf '\033]7;file://localhost%s\007' $encoded_pwd
end

function _doraterm_si_prompt --on-event fish_prompt
    set -l _doraterm_si_status $status
    _doraterm_si_blocked; and return
    if test $_DORATERM_SI_FIRSTPROMPT -eq 1
    ...
    set -g _DORATERM_SI_FIRSTPROMPT 0
end

function _doraterm_si_preexec --on-event fish_preexec
    _doraterm_si_blocked; and return
...
function _doraterm_si_chpwd --on-variable PWD
    _doraterm_si_osc7
end