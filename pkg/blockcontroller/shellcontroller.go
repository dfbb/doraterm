...
func getCustomInitScript(logCtx context.Context, meta doraobj.MetaMapType, connName string, shellType string) string {
	...
	if err != nil {
		blocklogger.Infof(logCtx, "[conndebug] cannot expand home dir in Dora initscript file: %v\n", err)
		return fmt.Sprintf("echo \"cannot expand home dir in Dora initscript file, from key %s\";\n", metaKeyName)
	}
	fileData, err := os.ReadFile(initScriptVal)
	if err != nil {
		blocklogger.Infof(logCtx, "[conndebug] cannot open Dora initscript file: %v\n", err)
		return fmt.Sprintf("echo \"cannot open Dora initscript file, from key %s\";\n", metaKeyName)
	}
