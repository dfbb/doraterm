...
import (
	...
	"github.com/dfbb/doraterm/pkg/dconfig/defaultconfig"
)
...
type FullConfigType struct {
	...
	Presets        map[string]doraobj.MetaMapType  `json:"presets"`
...
func resolveEnvReplacements(m doraobj.MetaMapType) {
	if m == nil {
		return
	}
	...
	for key, value := range m {
		...
		case map[string]interface{}:
			resolveEnvReplacements(doraobj.MetaMapType(v))
		case []interface{}:
			resolveEnvArray(v)
		}
	}
...
func readConfigHelper(fileName string, barr []byte, readErr error) (doraobj.MetaMapType, []ConfigError) {
...
func readConfigFileFS(fsys fs.FS, logPrefix string, fileName string) (doraobj.MetaMapType, []ConfigError) {
...
func ReadDefaultsConfigFile(fileName string) (doraobj.MetaMapType, []ConfigError) {
...
func mergeMetaMapSimple(m doraobj.MetaMapType, toMerge doraobj.MetaMapType) doraobj.MetaMapType {
	if m == nil {
		return toMerge
	}
	if toMerge == nil {
...
func mergeMetaMap(m doraobj.MetaMapType, toMerge doraobj.MetaMapType, simpleMerge bool) doraobj.MetaMapType {
	if simpleMerge {
		return mergeMetaMapSimple(m, toMerge)
	} else {
		return doraobj.MergeMeta(m, toMerge, true)
	}
}
...
func readConfigFilesForDir(fsys fs.FS, logPrefix string, dirName string, fileName string, simpleMerge bool) (doraobj.MetaMapType, []ConfigError) {
...
func readConfigPartForFS(fsys fs.FS, logPrefix string, partName string, simpleMerge bool) (doraobj.MetaMapType, []ConfigError) {
...
// in golang code, the best way to get the current config is via the watcher -- dconfig.GetWatcher().GetFullConfig()
func ReadFullConfig() FullConfigType {
	...
	for fieldIdx := 0; fieldIdx < configRType.NumField(); fieldIdx++ {
		...
		var configPart doraobj.MetaMapType
		var errs []ConfigError
		if jsonTag == "-" || jsonTag == "" {
			continue
		} else {
...
func orderConfigKeys(m doraobj.MetaMapType) []string {
...
func jsonMarshalConfigInOrder(m doraobj.MetaMapType) ([]byte, error) {
	if len(m) == 0 {
		return []byte("{}"), nil
	}
...
func SetConnectionsConfigValue(connName string, toMerge doraobj.MetaMapType) error {
	...
	if connData == nil {
		connData = make(doraobj.MetaMapType)
	}
	for configKey, val := range toMerge {
