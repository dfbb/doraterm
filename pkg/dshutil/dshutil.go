...
func MakeClientJWTToken(rpcCtx dshrpc.RpcContext) (string, error) {
...
func ValidateAndExtractRpcContextFromToken(tokenStr string) (*dshrpc.RpcContext, error) {
	claims, err := dorajwt.ValidateAndExtract(tokenStr)
	if err != nil {
		return nil, err
	}
...
func GetInfo() dshrpc.RemoteInfo {
...
func SendErrCh[T any](err error) <-chan dshrpc.RespOrErrorUnion[T] {
	ch := make(chan dshrpc.RespOrErrorUnion[T], 1)
	ch <- RespErr[T](err)
	close(ch)
	return ch
}

func RespErr[T any](err error) dshrpc.RespOrErrorUnion[T] {
	return dshrpc.RespOrErrorUnion[T]{Error: err}
}
