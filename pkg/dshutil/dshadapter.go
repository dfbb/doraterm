...
func serverImplAdapter(impl any) func(*RpcResponseHandler) bool {
	...
	return func(handler *RpcResponseHandler) bool {
		...
		if rmethod == nil {
			if !handler.NeedsResponse() && cmd != dshrpc.Command_Message {
		...
		if len(commandDataTypes) == 1 {
			...
			multiArg, ok := multiArgAny.(dshrpc.MultiArg)
			if !ok {
		...
		if methodDecl.CommandType == dshrpc.RpcType_Call {
			...
			return true
		} else if methodDecl.CommandType == dshrpc.RpcType_ResponseStream {
			rtnVals := implMethod.Call(callParams)
			rtnChVal := rtnVals[0]
			if rtnChVal.IsNil() {
