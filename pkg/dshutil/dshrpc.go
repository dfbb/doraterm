...
type DshRpc struct {
	...
	RpcContext         *atomic.Pointer[dshrpc.RpcContext]
...
func (w *DshRpc) handleStreamAck(req *RpcMessage) {
	...
	var ackPk dshrpc.CommandStreamAckData
...
outer:
	for {
		...
		if msg.IsRpcRequest() {
			...
			if msg.Command == dshrpc.Command_StreamData {
				w.handleStreamData(&msg)
				continue
			}
			if msg.Command == dshrpc.Command_StreamDataAck {
...
func (handler *RpcResponseHandler) GetRpcContext() dshrpc.RpcContext {
...
func (handler *RpcResponseHandler) SendMessage(msg string) {
	rpcMsg := &RpcMessage{
		...
		Data: dshrpc.CommandMessageData{
			Message: msg,
		},
