...
func (impl *ServerImpl) RemoteFileCopyCommand(ctx context.Context, data dshrpc.CommandFileCopyData) (bool, error) {
	...
	destPathCleaned := filepath.Clean(dorabase.ExpandHomeDirSafe(destConn.Path))
	destHasSlash := strings.HasSuffix(data.DestUri, "/")

	if srcConn.Host == destConn.Host {
		srcPathCleaned := filepath.Clean(dorabase.ExpandHomeDirSafe(srcConn.Path))
	...
	streamData := dshrpc.CommandRemoteFileStreamData{
		Path:       srcConn.Path,
...
func (impl *ServerImpl) RemoteListEntriesCommand(ctx context.Context, data dshrpc.CommandRemoteListEntriesData) chan dshrpc.RespOrErrorUnion[dshrpc.CommandRemoteListEntriesRtnData] {
	ch := make(chan dshrpc.RespOrErrorUnion[dshrpc.CommandRemoteListEntriesRtnData], 16)
	go func() {
		defer func() {
			panichandler.PanicHandler("RemoteListEntriesCommand", recover())
		}()
		defer close(ch)
		path, err := dorabase.ExpandHomeDir(data.Path)
		if err != nil {
			ch <- dshutil.RespErr[dshrpc.CommandRemoteListEntriesRtnData](err)
			return
		}
		if data.Opts == nil {
			data.Opts = &dshrpc.FileListOpts{}
		}
		innerFilesEntries := []os.DirEntry{}
		seen := 0
		if data.Opts.Limit == 0 {
			data.Opts.Limit = dshrpc.MaxDirSize
		}
		if data.Opts.All {
			if DisableRecursiveFileOpts {
				ch <- dshutil.RespErr[dshrpc.CommandRemoteListEntriesRtnData](fmt.Errorf("recursive directory listings are not supported"))
				return
			}
			...
			if err != nil {
				ch <- dshutil.RespErr[dshrpc.CommandRemoteListEntriesRtnData](fmt.Errorf("cannot open dir %q: %w", path, err))
				return
			}
		}
		var fileInfoArr []*dshrpc.FileInfo
		for _, innerFileEntry := range innerFilesEntries {
			if ctx.Err() != nil {
				ch <- dshutil.RespErr[dshrpc.CommandRemoteListEntriesRtnData](ctx.Err())
				return
			}
			...
			if len(fileInfoArr) >= dshrpc.DirChunkSize {
				resp := dshrpc.CommandRemoteListEntriesRtnData{FileInfo: fileInfoArr}
				ch <- dshrpc.RespOrErrorUnion[dshrpc.CommandRemoteListEntriesRtnData]{Response: resp}
				fileInfoArr = nil
			}
		}
...
func statToFileInfo(fullPath string, finfo fs.FileInfo, extended bool) *dshrpc.FileInfo {
	mimeType := fileutil.DetectMimeType(fullPath, finfo, extended)
	rtn := &dshrpc.FileInfo{
		Path:          dorabase.ReplaceHomeDir(fullPath),
...
func computeDirPart(path string) string {
	path = filepath.Clean(dorabase.ExpandHomeDirSafe(path))
...
func (*ServerImpl) fileInfoInternal(path string, extended bool) (*dshrpc.FileInfo, error) {
	cleanedPath := filepath.Clean(dorabase.ExpandHomeDirSafe(path))
	finfo, err := os.Stat(cleanedPath)
	if os.IsNotExist(err) {
		return &dshrpc.FileInfo{
			Path:          dorabase.ReplaceHomeDir(path),
...
func resolvePaths(paths []string) string {
	if len(paths) == 0 {
		return dorabase.ExpandHomeDirSafe("~")
	}
	rtnPath := dorabase.ExpandHomeDirSafe(paths[0])
	for _, path := range paths[1:] {
		path = dorabase.ExpandHomeDirSafe(path)
...
func (impl *ServerImpl) RemoteFileJoinCommand(ctx context.Context, paths []string) (*dshrpc.FileInfo, error) {
	rtnPath := resolvePaths(paths)
	return impl.fileInfoInternal(rtnPath, true)
}

func (impl *ServerImpl) RemoteFileInfoCommand(ctx context.Context, path string) (*dshrpc.FileInfo, error) {
	return impl.fileInfoInternal(path, true)
}

func (impl *ServerImpl) RemoteFileMultiInfoCommand(ctx context.Context, data dshrpc.CommandRemoteFileMultiInfoData) (map[string]dshrpc.FileInfo, error) {
	cwd := data.Cwd
	if cwd == "" {
		cwd = "~"
	}
	cwd = filepath.Clean(dorabase.ExpandHomeDirSafe(cwd))
	rtn := make(map[string]dshrpc.FileInfo, len(data.Paths))
	for _, path := range data.Paths {
		...
		cleanedPath := dorabase.ExpandHomeDirSafe(path)
		if !filepath.IsAbs(cleanedPath) {
			cleanedPath = filepath.Join(cwd, cleanedPath)
		}
		fileInfo, err := impl.fileInfoInternal(cleanedPath, false)
		if err != nil {
			rtn[path] = dshrpc.FileInfo{
				Path:          dorabase.ReplaceHomeDir(cleanedPath),
...
func (impl *ServerImpl) RemoteFileMoveCommand(ctx context.Context, data dshrpc.CommandFileCopyData) error {
...
func (*ServerImpl) RemoteWriteFileCommand(ctx context.Context, data dshrpc.FileData) error {
	...
	path, err := dorabase.ExpandHomeDir(data.Info.Path)
	if err != nil {
		return err
	}
...
func (impl *ServerImpl) RemoteFileStreamCommand(ctx context.Context, data dshrpc.CommandRemoteFileStreamData) (*dshrpc.FileInfo, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			writer.Close()
			return &dshrpc.FileInfo{
				Path:     dorabase.ReplaceHomeDir(data.Path),
...
func (*ServerImpl) RemoteFileDeleteCommand(ctx context.Context, data dshrpc.CommandDeleteFileData) error {
	expandedPath, err := dorabase.ExpandHomeDir(data.Path)
	if err != nil {
