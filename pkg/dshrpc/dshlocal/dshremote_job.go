...
func (impl *ServerImpl) connectToJobManager(ctx context.Context, jobId string, mainServerJwtToken string) (string, func(), error) {
	...
	proxy := dshutil.MakeRpcProxy("jobmanager")
	...
	go func() {
		writeErr := dshutil.AdaptOutputChToStream(proxy.ToRemoteCh, conn)
		if writeErr != nil {
	...
	go func() {
		...
		dshutil.AdaptStreamToMsgCh(conn, proxy.FromRemoteCh, nil)
	}()

	routeId := dshutil.MakeLinkRouteId(linkId)
	authData := dshrpc.CommandAuthenticateToJobData{
		JobAccessToken: mainServerJwtToken,
	}
	err = dshclient.AuthenticateToJobManagerCommand(impl.RpcClient, authData, &dshrpc.RpcOpts{Route: routeId})
	if err != nil {
		cleanup()
		return "", nil, fmt.Errorf("authentication to job manager failed: %w", err)
	}

	jobRouteId := dshutil.MakeJobRouteId(jobId)
...
func (impl *ServerImpl) RemoteStartJobCommand(ctx context.Context, data dshrpc.CommandRemoteStartJobData) (*dshrpc.CommandStartJobRtnData, error) {
	...
	startJobData := dshrpc.CommandStartJobData{
		Cmd:        data.Cmd,
		Args:       data.Args,
		Env:        combinedEnv,
	...
	rtnData, err := dshclient.StartJobCommand(impl.RpcClient, startJobData, &dshrpc.RpcOpts{Route: jobRouteId})
	if err != nil {
		cleanup()
...
func (impl *ServerImpl) RemoteReconnectToJobManagerCommand(ctx context.Context, data dshrpc.CommandRemoteReconnectToJobManagerData) (*dshrpc.CommandRemoteReconnectToJobManagerRtnData, error) {
	log.Printf("RemoteReconnectToJobManagerCommand: reconnecting, jobid=%s\n", data.JobId)
	if impl.Router == nil {
		return &dshrpc.CommandRemoteReconnectToJobManagerRtnData{
			Success: false,
...
func (impl *ServerImpl) RemoteDisconnectFromJobManagerCommand(ctx context.Context, data dshrpc.CommandRemoteDisconnectFromJobManagerData) error {
...
func (impl *ServerImpl) RemoteTerminateJobManagerCommand(ctx context.Context, data dshrpc.CommandRemoteTerminateJobManagerData) error {
