...
func parseSimpleId(simpleId string) (discriminator string, value string, err error) {
	...
	if _, err := doraobj.ParseORef(simpleId); err == nil {
...
func resolveThis(ctx context.Context, data dshrpc.CommandResolveIdsData, value string) (*doraobj.ORef, error) {
	if data.BlockId == "" {
		return nil, fmt.Errorf("no blockid in request")
	}

	if value == SimpleId_This || value == SimpleId_Block {
		return &doraobj.ORef{OType: doraobj.OType_Block, OID: data.BlockId}, nil
	}
	if value == SimpleId_Tab {
		...
		return &doraobj.ORef{OType: doraobj.OType_Tab, OID: tabId}, nil
	}
	if value == SimpleId_Ws || value == SimpleId_Workspace {
		...
		return &doraobj.ORef{OType: doraobj.OType_Workspace, OID: wsId}, nil
	}
	if value == SimpleId_Client || value == SimpleId_Global {
		clientId := dstore.GetClientId()
		return &doraobj.ORef{OType: doraobj.OType_Client, OID: clientId}, nil
	}
	if value == SimpleId_Temp {
		...
		return &doraobj.ORef{OType: "temp", OID: client.TempOID}, nil
	}
	return nil, fmt.Errorf("invalid value for 'this' resolver: %s", value)
}

func resolveORef(_ context.Context, value string) (*doraobj.ORef, error) {
	parsedORef, err := doraobj.ParseORef(value)
	if err != nil {
...
func resolveTabNum(ctx context.Context, data dshrpc.CommandResolveIdsData, value string) (*doraobj.ORef, error) {
	...
	curTabId, err := dstore.DBFindTabForBlockId(ctx, data.BlockId)
	if err != nil {
		return nil, fmt.Errorf("error finding tab for block: %v", err)
	}

	wsId, err := dstore.DBFindWorkspaceForTabId(ctx, curTabId)
	if err != nil {
		return nil, fmt.Errorf("error finding current workspace: %v", err)
	}

	ws, err := dstore.DBMustGet[*doraobj.Workspace](ctx, wsId)
	if err != nil {
	...
	return &doraobj.ORef{OType: doraobj.OType_Tab, OID: resolvedTabId}, nil
}

func resolveBlock(ctx context.Context, data dshrpc.CommandResolveIdsData, value string) (*doraobj.ORef, error) {
	...
	layout, err := dstore.DBGet[*doraobj.LayoutState](ctx, tab.LayoutState)
	if err != nil {
	...
	return &doraobj.ORef{OType: doraobj.OType_Block, OID: leafEntry.BlockId}, nil
}

func resolveView(ctx context.Context, data dshrpc.CommandResolveIdsData, value string) (*doraobj.ORef, error) {
	...
	layout, err := dstore.DBMustGet[*doraobj.LayoutState](ctx, tab.LayoutState)
	if err != nil {
	...
	for _, leaf := range *layout.LeafOrder {
		...
		leafBlock, err := dstore.DBMustGet[*doraobj.Block](ctx, leafBlockId)
		if err != nil {
			continue
		}
		if leafBlock.Meta.GetString("view", "") == viewType {
			...
			if count == instanceNum {
				return &doraobj.ORef{OType: doraobj.OType_Block, OID: leaf.BlockId}, nil
			}
		}
	}
...
func resolveUUID(ctx context.Context, value string) (*doraobj.ORef, error) {
	return dstore.DBResolveEasyOID(ctx, value)
}

// Main resolver function
func resolveSimpleId(ctx context.Context, data dshrpc.CommandResolveIdsData, simpleId string) (*doraobj.ORef, error) {
