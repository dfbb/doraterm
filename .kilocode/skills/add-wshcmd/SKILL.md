---
name: add-wshcmd
description: Guide for adding new wsh commands to Dora Terminal. Use when implementing new CLI commands, adding command-line functionality, or extending the wsh command interface.
---

# Adding a New wsh Command to Dora Terminal

This guide explains how to add a new command to the `wsh` CLI tool.

## wsh Command System Overview

Dora Terminal's `wsh` command provides CLI access to Dora Terminal features. The system uses:
...
3. **RPC Client** - Communication with Dora Terminal backend via `RpcClient`
...
import (
    ...
    "github.com/doratermdev/doraterm/pkg/wshrpc"
    "github.com/doratermdev/doraterm/pkg/wshrpc/wshclient"
)
...
**Use case:** A command that prints Dora Terminal version info
...
import (
    "github.com/spf13/cobra"
    "github.com/doratermdev/doraterm/pkg/wavebase"
)

var versionCmd = &cobra.Command{
    Use:   "version",
    Short: "Print Dora Terminal version",
    RunE:  versionRun,
}
...
Print the current Dora Terminal version.
