...
func init() {
	...
	tabIndicatorCmd.Flags().StringVar(&tabIndicatorTabId, "tabid", "", "tab id (defaults to DORATERM_TABID)")
...
func tabIndicatorRun(cmd *cobra.Command, args []string) (rtnErr error) {
	...
	if tabId == "" {
		return fmt.Errorf("no tab id specified (use --tabid or set DORATERM_TABID)")
	}
