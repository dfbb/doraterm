...
function createModel(value: string, path: string, language?: string) {
    const uri = monaco.Uri.parse(`dora://editor/${encodeURIComponent(path)}`);
...
export function MonacoDiffViewer({ original, modified, language, path, options }: DiffViewerProps) {
    ...
    useEffect(() => {
        ...
        const origUri = monaco.Uri.parse(`dora://diff/${encodeURIComponent(path)}.orig`);
        const modUri = monaco.Uri.parse(`dora://diff/${encodeURIComponent(path)}.mod`);
