...
export type BadgeEnv = DoraEnvSubset<{
    rpc: {
        EventPublishCommand: DoraEnv["rpc"]["EventPublishCommand"];
    };
}>;

export type LoadBadgesEnv = DoraEnvSubset<{
    rpc: {
        GetAllBadgesCommand: DoraEnv["rpc"]["GetAllBadgesCommand"];
    };
}>;

export type TabBadgesEnv = DoraEnvSubset<{
    wos: DoraEnv["wos"];
}>;
...
function publishBadgeEvent(eventData: DoraEvent, env?: BadgeEnv) {
    if (env != null) {
...
function clearBadgesForTab(tabId: string, env?: BadgeEnv) {
    const tabAtom = WOS.getDoraObjectAtom<Tab>(WOS.makeORef("tab", tabId));
...
function getTabBadgeAtom(tabId: string, env?: TabBadgesEnv): Atom<Badge[]> {
    ...
    const tabAtom = env != null ? env.wos.getDoraObjectAtom<Tab>(tabOref) : WOS.getDoraObjectAtom<Tab>(tabOref);
    rtn = atom((get) => {
