# Dora Terminal OSC 16162 Escape Sequences

Dora Terminal uses a custom OSC (Operating System Command) escape sequence numbered **16162** for shell integration. This allows the shell to communicate its state and events to the terminal.
...
**Purpose:** Signals to the terminal that a new prompt is being drawn. This helps Dora Terminal distinguish between prompt output and command output.
...
**Purpose:** Provides Dora Terminal with information about the shell environment and operating system.
...
**Purpose:** Communicates whether the previous command succeeded or failed, allowing Dora Terminal to display success/failure indicators.
...
**Purpose:** Allows Dora Terminal to track the state of the command line input. Currently reports whether the buffer is empty, but may be extended to include additional input state information in the future.
...
Dora Terminal also uses the standard **OSC 7** sequence for reporting the current working directory:
